...
def activation_distance(entry, atr, thresholds, *, taker_fee=.0005, slippage=.001):
...
def floor_plan(side, entry, current, peak, initial_stop, atr, *, taker_fee=.0005,
               slippage=.001, thresholds=None):
    ...
    costs = entry * (2 * taker_fee + 2 * slippage)
    activation = activation_distance(entry, atr, ex, taker_fee=taker_fee, slippage=slippage)
    if gain < activation:
...
def allow_ai_tightening(side, entry, current, new_stop, atr, *, taker_fee=.0005,
                        slippage=.001, thresholds=None):
    ex = thresholds if thresholds is not None else exit_policy.thresholds('standard')
